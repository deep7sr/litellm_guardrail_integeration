...
Registered in config.guardrail.yaml as a custom guardrail firing on
...
logic runs inside this LiteLLM process — no separate scoring service.
  ...
  4. Logs every scoring attempt to Postgres (faithfulness_events).

Integration contract: RAG apps must send
  extra_body={"metadata": {
      ...
      "guardrail_threshold": 0.6,                         # optional override
  }}
...
# NOTE: reuses this project's existing env var names (GUARDRAIL_THRESHOLD,
# MAX_ATTEMPTS, GUARDRAIL_MODE, JUDGE_MODEL — already set in .env) instead of
...
async def _log_event(req_id, attempt, score, verdict, target_model, question, answer):
    ...
    try:
        async with pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO faithfulness_events (req_id, attempt, score, verdict, target_model, question, answer_snippet)
...
class FaithfulnessGuardrail(CustomGuardrail):
    ...
    async def async_post_call_success_hook(self, data: dict, user_api_key_dict: UserAPIKeyAuth, response):
        ...
        if contexts is None:
            ...
            logger.info(f"[faithfulness] req={req_id} no guardrail_context supplied, passing unscored")
